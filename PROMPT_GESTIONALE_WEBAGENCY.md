...
32. **Cestino dei task** — eliminare un task è un soft-delete (`Task.deletedAt`): sparisce da Kanban, dashboard, calendario, "Il mio lavoro", ricerca e dettaglio progetto, ma resta nella vista "Cestino" di `/task` (toggle in header, con conteggio) da cui **ripristinare** o eliminare definitivamente (con conferma). Il lazy-cron dei rinnovi non rigenera i promemoria cestinati di proposito. **Svuotamento automatico dopo 30 giorni** (`src/lib/trash-purge.ts`, lazy-cron in dashboard; avviso nella vista). Scelta deliberata: il cestino resta SOLO sui task — clienti/progetti hanno già l'archivio (stati CLOSED/ARCHIVIATO), i preventivi persi vanno marcati RIFIUTATO, il resto ha conferme/permessi; estenderlo ovunque duplicherebbe protezioni esistenti e moltiplicherebbe i punti da filtrare.

33. **Pagina Log (solo ADMIN)** — `/log`: registro attività completo, paginato (50/pagina) e filtrabile per utente, con link all'entità collegata. Pulizia per sfoltire il DB: eliminazione singola, a selezione multipla (checkbox + "seleziona tutto"), e in blocco ("più vecchi di 6 mesi" / "svuota tutto", con conferma). Eliminazione definitiva (è storico, niente cestino); enforcement ADMIN in pagina e in ogni action.
